...
public class dataMembres extends Connect {
    ...
    public membres selectMembres(int numMembre) {
        try {
            String query = "SELECT * FROM membres WHERE num_membre = ?;";
...
public DefaultTableModel getAllMembre() throws SQLException {
    ...
    try (PreparedStatement pst = connection.prepareStatement(query)) {
        ...
        while (resultSet.next()) {
            int id = resultSet.getInt("num_membre");
            String nom = resultSet.getString("nom");
            String prenom = resultSet.getString("prenom");
            String email = resultSet.getString("email");
            String Action = "";
            // Ajout des informations du membre au modèle de table
            model.addRow(new Object[]{id, nom, prenom, email, Action});
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
